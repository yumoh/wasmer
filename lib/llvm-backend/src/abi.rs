...
// the user pack and unpack values itself sometimes. This can help the LLVM
// optimizer by exposing operations to the optimizer, but it requires that the
// frontend produce unnatural IR in order to get the right ABI.
//
...
pub fn func_sig_to_llvm<'ctx>(
    ...
    match sig_returns_bitwidths.as_slice() {
        ...
        [32, 32] if sig.returns()[0] == Type::F32 && sig.returns()[1] == Type::F32 => (
            intrinsics
                .f32_ty
                .vec_type(2)
                .fn_type(&param_types.collect::<Vec<_>>(), false),
        ...
        [32, 32, _] if sig.returns()[0] == Type::F32 && sig.returns()[1] == Type::F32 => (
            context
                .struct_type(
                    &[
        ...
        [64, 32, 32] if sig.returns()[1] == Type::F32 && sig.returns()[2] == Type::F32 => (
            context
                .struct_type(
                    &[
