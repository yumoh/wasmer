...
use wasm_common::WASM_MAX_PAGES;
use wasm_common::{
...
impl MemoryStyle {
    ...
    pub fn for_memory(memory: MemoryType, tunables: &Tunables) -> (Self, u64) {
...
impl MemoryPlan {
    ...
    pub fn for_memory(memory: MemoryType, tunables: &Tunables) -> Self {
...
impl TableStyle {
    ...
    pub fn for_table(_table: TableType, _tunables: &Tunables) -> Self {
...
impl TablePlan {
    ...
    pub fn for_table(table: TableType, tunables: &Tunables) -> Self {
...
impl Module {
        ...
        self.exports
            ...
            .filter_map(|(_name, export_index)| match export_index {
                ExportIndex::Function(i) => {
                    let signature = self.local.functions.get(i.clone()).unwrap();
                    let func_type = self.local.signatures.get(signature.clone()).unwrap();
        ...
        self.exports
            ...
            .map(|(name, export_index)| {
                let extern_type = match export_index {
                    ...
                    ExportIndex::Table(i) => {
                        let table_plan = self.local.table_plans.get(i.clone()).unwrap();
                        ExternType::Table(table_plan.table)
                    }
                    ExportIndex::Memory(i) => {
                        let memory_plan = self.local.memory_plans.get(i.clone()).unwrap();
                        ExternType::Memory(memory_plan.memory)
                    }
                    ExportIndex::Global(i) => {
                        let global_type = self.local.globals.get(i.clone()).unwrap();
    ...
    pub fn imports(&self) -> Vec<ImportType> {
        self.imports
            .iter()
            .map(|((module, field, _), import_index)| {
